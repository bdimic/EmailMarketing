...
<sf:form method="POST" action="${pageContext.request.contextPath}/pickCampaignAction" commandName="clickResponse">
<div class="sadrzaj">
<table id="campaigns_table" cellpadding="0" cellspacing="0" border="0"  class="display">
    <thead>
        <tr>
            <th><spring:message code="clickResponse.email" /></th>
            <th><spring:message code="clickResponse.response_source" /></th>
            <th><spring:message code="clickResponse.response_url" /></th>
            <th><spring:message code="clickResponse.response_time" /></th>
        </tr>
    </thead>
    <tbody>
        <c:forEach var="clickResponse" items="${clickResponse}">
        <tr>
            <td>${clickResponse.email}</td>
            <td>${clickResponse.response_source}</td>
            <td>${clickResponse.response_url}</td>
            <td>${clickResponse.response_time}</td>
        </tr>
        </c:forEach>
    </tbody>
</table>
